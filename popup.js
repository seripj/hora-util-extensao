...
const I18N = {
  pt: {
    tab_mes: "Por Mês",
    tab_periodo: "Por Período",
    label_mes: "Mês / Ano",
    label_inicio: "Data Início",
    label_fim: "Data Fim",
    label_folgas: "🏖️ Folgas + Feriados",
    label_horas: "⏰ Horas/Dia",
    divider_resultados: "resultados",
    res_uteis_label: "Dias Úteis no Período",
    res_corridos_label: "Dias Corridos",
    res_uteis_hoje_label: "Úteis até Hoje",
    res_uteis_hoje_sub: "dias trabalhados",
    res_horas_total_label: "Horas a Baixar",
    res_horas_total_sub: "total do período",
    res_horas_hoje_label: "Horas até Hoje",
    footer_data: "Data atual:",
    sem_desconto: "sem descontos aplicados",
    ate_hoje: "até hoje",
    ate_data: (d) => `até ${d}`,
    fim_do_dia: "fim do dia de hoje",
    dias_uteis: (n) =>
      `${n} dia${n !== 1 ? "s" : ""} útei${n !== 1 ? "s" : ""}`,
  ...
  en: {
    tab_mes: "By Month",
    tab_periodo: "By Period",
    label_mes: "Month / Year",
    label_inicio: "Start Date",
    label_fim: "End Date",
    label_folgas: "🏖️ Days Off + Holidays",
    label_horas: "⏰ Hours/Day",
    divider_resultados: "results",
    res_uteis_label: "Working Days in Period",
    res_corridos_label: "Calendar Days",
    res_uteis_hoje_label: "Working Days to Date",
    res_uteis_hoje_sub: "days worked",
    res_horas_total_label: "Hours to Log",
    res_horas_total_sub: "total for period",
    res_horas_hoje_label: "Hours to Date",
    footer_data: "Today:",
    sem_desconto: "no discounts applied",
    ate_hoje: "through today",
    ate_data: (d) => `through ${d}`,
    fim_do_dia: "end of today",
...
function buildHolidaySet(inicio, fim) {
  ...
  anos.forEach((ano) => {
    ...
    [
      ...
      todas.push({ data: new Date(ano, m - 1, d), nome: n }),
    );
    [
      [-48, "Carnaval (2ª-feira)"],
      [-47, "Carnaval (3ª-feira)"],
      [-2, "Sexta-feira Santa"],
      [0, "Páscoa"],
      [60, "Corpus Christi"],
  ...
  todas.forEach(({ data, nome }) => {
    ...
    if (data >= inicio && data <= fim && dow !== 0 && dow !== 6)
      map.set(toISO(data), nome);
  });
  return map;
}
...
function openDP(which) {
  ...
  if (which === "mes") {
    ...
    if (ref) {
      ...
      dpViewYear = hoje.getFullYear();
      dpViewMonth = hoje.getMonth();
    }
  }
  renderDP();
...
function renderMonthPicker(popup) {
  ...
  for (let m = 0; m < 12; m++) {
    const isSel =
      selMesAno && selMesAno.year === dpViewYear && selMesAno.month === m;
...
function refreshTriggerLabels() {
  ...
  if (btnInicioText)
    btnInicioText.textContent = selInicio
      ? formatDate(selInicio)
      : t("placeholder_data");
  // Fim
  const btnFimText = $("btn-fim-text");
  if (btnFimText)
    btnFimText.textContent = selFim
      ? formatDate(selFim)
      : t("placeholder_data");
}
...
function calcular() {
  ...
  let diasCorridos = 0,
    ...
    horasHoje = 0;
  if (dataBase) {
    const feriadosAteBase = buildHolidaySet(inicio, dataBase).size;
    const folgasAteHoje = Math.min(folgas, feriadosAteBase);

    diasCorridos = contarDiasCorridos(inicio, dataBase);
    diasUteisHoje = Math.max(
      0,
      contarDiasUteisBase(inicio, dataBase) - folgasAteHoje,
    );
  ...
  if (qtdFeriados > 0) {
    ...
    partes.push(
      folgas !== qtdFeriados ? `${ferStr} (${t("ajustado", folgas)})` : ferStr,
    );
  }
  $("res-uteis-sub").textContent =
    partes.length > 1
  ...
  if (dataBase)
    $("res-corridos-sub").textContent = dentroDoPeriodo
      ? t("ate_hoje")
      : t("ate_data", formatDate(fim));

  setResult("res-horas-total", `${horasTotal}h`);
  setResult("res-horas-hoje", dataBase ? `${horasHoje}h` : "—");
  if (dataBase)
    $("res-horas-hoje-sub").textContent = dentroDoPeriodo
      ? t("fim_do_dia")
      : t("ate_data", formatDate(fim));
  ...
  try {
    chrome.storage.local.set({
      ...
      activeTab,
      selMesAno,
      selInicio: selInicio ? toISO(selInicio) : null,
      selFim: selFim ? toISO(selFim) : null,
      folgas: $("folgas").value,
      horasDia: $("horas-dia").value,
      folgasEditadoManualmente,
    });
  } catch (_) {}
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  $("folgas").addEventListener("input", () => {
    folgasEditadoManualmente = true;
    calcular();
  });
  $("folgas").addEventListener("change", () => {
    folgasEditadoManualmente = true;
    calcular();
  });
